...
> **先備知識提示**：本週涉及的反函數與對數基本定義，可複習 [微積分(上) 第 13 週課程](../../calculus-i/week13.md)。
...
1.  **積分上下限的順序**：在計算 $\int_a^b f(x) dx$ 時，若 $a > b$，結果會是 $-\int_b^a f(x) dx$。在物理應用（如做功）中，方向非常重要。
2.  **反函數的誤區**：$f^{-1}(x)$ 是反函數，並非倒數 $1/f(x)$。例如 $(\sin x)^{-1}$ 通常指 $\arcsin x$，而 $\csc x$ 才是 $1/\sin x$。
3.  **對數底數的混淆**：在微積分中，若無特別說明，$\log x$ 通常指自然對數 $\ln x$。但在工程領域可能指 $\log_{10}$。務必確認上下文。
