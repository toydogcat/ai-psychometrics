...
1.  **數列 vs. 級數**：數列 $\{a_n\}$ 的收斂是指當 $n \to \infty$ 時 $a_n$ 趨近於常數；而級數 $\sum a_n$ 的收斂是指其累加和趨近於常數。不可將兩者混為一談。
2.  **有界不代表收斂**：例如數列 $a_n = (-1)^n$ 是有界的（在 -1 與 1 之間），但它並不收斂，而是在兩個值之間震盪。收斂需要趨向單一極限。
3.  **單調不代表收斂**：單調數列必須同時具備「有界性」才能保證收斂。例如 $a_n = n$ 是單調遞增的，但因為無上界，所以發散。
