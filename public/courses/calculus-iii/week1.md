...
1.  **外積的反交換律**：外積不滿足交換律，即 $\mathbf{a} \times \mathbf{b} \neq \mathbf{b} \times \mathbf{a}$。正確關係是 $\mathbf{a} \times \mathbf{b} = -(\mathbf{b} \times \mathbf{a})$。這在右手定則判定中極其重要。
2.  **純量與向量的混淆**：內積 $\mathbf{a} \cdot \mathbf{b}$ 的結果是**純量**，而外積 $\mathbf{a} \times \mathbf{b}$ 的結果是**向量**。不可混淆兩者的運算結果類型。
3.  **距離公式的誤用**：點到平面的距離公式涉及法向量，務必確保法向量 $\mathbf{n} = \langle a, b, c \rangle$ 是從平面方程式 $ax+by+cz=d$ 中正確提取的。
